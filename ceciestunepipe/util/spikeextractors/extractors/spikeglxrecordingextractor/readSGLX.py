...
logger = logging.getLogger('ceciestunepipe.util.spikeextractors.extractors.readSGLX')
...
def ExtractDigital(rawData, firstSamp, lastSamp, dwReq, dLineList, meta):
    ...
    if meta['typeThis'] == 'imec':
        logger.info('Extracting digital on imec stream: there is only one channel, if any (SYN) and will be converted from an int16 channel ')
        dig_array = extract_imec_syn_ch(rawData, firstSamp, lastSamp, meta)
    else:
        MN, MA, XA, DW = ChannelCountsNI(meta)
        if dwReq > DW-1:
            ...
            nLine = len(dLineList)
            dig_array = np.zeros((nLine, nSamp), 'uint8')
            for i in range(0, nLine):
                byteN, bitN = np.divmod(dLineList[i], 8)
                targI = byteN*8 + (7 - bitN)
                dig_array[i, :] = bitWiseData[targI, :]
    
    return dig_array

def extract_imec_syn_ch(raw_data: np.array, first_samp:int, last_samp:int, meta_dict:dict) -> np.array:
    ## extract the 
